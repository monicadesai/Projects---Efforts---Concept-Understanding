...
# Aim - Score = 212467/RMSE which should be greater than o.51

# get the working directory --------------------------------------------------------------------------------------------------------------------
getwd()

## set the working directory where to work and your data exists
setwd("C:\\Users\\Monica\\Desktop\\Projects\\R Projects\\Project1")
...
#install.packages('dplyr') (keep your net on while installing libraries)
library(dplyr)
...
CreateDummies=function(data,var,freq_cutoff=0){
  ...
  t=t[t>freq_cutoff] ## cutoff is the frequency of occurrence of a variable default is 0 , but ideally it should be at least 15-20% of actual data,
  ## so here whatever categories which are less than that cut off frequencies are dropped(no dummies are created for them)
  ...
  for( cat in categories){
    name=paste(var,cat,sep="_") ## Inside the for loop create a name separated by name of variable and category separated by "_" underscore
    name=gsub(" ","",name) ## replace any spaces if there is found in categories of variables
    name=gsub("-","_",name) ## replace any dash if found in categories to underscroes: e.g. 'Cat-1', 'Cat-2' will be 'Cat_1', 'Cat_2'
...
dim(house_train1)
dim(house_train2)
#install.packages('gbm') - (Keep your net on while installing libraries always)
library(gbm)
...
# combination. It does not have to be always 10
myerror=9999999
for(i in 1:num_trials){
  print(paste0('starting iteration:',i))
  # un-comment the line above to keep track of progress
  params=my_params[i,]
